...
\justifying
\noindent\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[frame hidden, boxrule=0pt, breakable, sharp corners, borderline west={3pt}{0pt}{shadecolor}, interior hidden, enhanced]}{\end{tcolorbox}}\fi
...
(iid) random variables \(\{X_1,\ldots,X_n\}\) with common cumulative
density function (cdf) \(F\) what is the limiting distribution of
\(M_n = \max\{X_1,\ldots,X_n\}\)? This is answered by the
...
then \(F\) is said to be in the domain of attraction of \(G\), denoted
...
These three types of distribution can be collected into one more general
distribution, called the generalised extreme value (GEV) distribution
...
Identifying what domain of attraction a distribution belongs to via
Theorem~\ref{thm-evt} may not be so trivial, however, there are other
ways to determine what domain of attraction a distribution belongs to.

\begin{definition}[Conditions for Domains of
Attraction]\protect\hypertarget{def-doa}{}\label{def-doa}

The distribution \(F\) belongs to the Fréchet domain of attraction
\(\mathcal D(\Phi_\alpha)\) if and only if the survival function
\(\bar F = 1-F\) is regularly varying with index \(-\alpha\) i.e.: \[
...
The condition for the Gumbel domain of attraction is not defined through
regular variation but instead, a distribution \(F\) belongs to the
Gumbel domain if and only if there exists a positive function
\(a: \mathbb R \rightarrow \mathbb R^+\) and a \(t\in \mathbb R\) such
that: \[
...
to the block maxima and threshold exceedances respectively. Typically,
when fitting the GP, a sufficiently high threshold needs to be specified
...
\phi\left[\left(1+\frac{\xi(y+1-v)}{\sigma_0+\xi v}\right)_+^{-1/\xi}-\left(1+\frac{\xi(y-v)}{\sigma_0+\xi v}\right)_+^{-1/\xi}\right]
\end{cases}
\]
