...
%}
\justifying
\noindent\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[breakable, boxrule=0pt, sharp corners, borderline west={3pt}{0pt}{shadecolor}, enhanced, interior hidden, frame hidden]}{\end{tcolorbox}}\fi
...
Denoted by \(\GEV(\mu,\sigma,\xi)\) the distribution is characterised by
three parameters \(\mu \in \mathbb R\) the location,
...
This quantity will play an important role in \textbf{?@sec-meth} when
...
\textbf{?@sec-meth} is the degree distribution of networks.
...
\item
  ...
  proportional to a function of their degree(minus one)\footnote{The
    ...
    the results from {[}GPA REF{]} \# Methods \{\#sec-meth\}} i.e.: \[
  E_t  = E_{t-1} \cup \tilde E
  \] where \(\tilde E = \{\tilde e_1,\ldots, \tilde e_m\}\) and
\end{enumerate}
